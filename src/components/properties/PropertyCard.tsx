...
import { Bed, Bath, Car, Maximize2, MapPin, Tag, ArrowRight } from 'lucide-react'
...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-card-hover transition-all duration-300 border border-gray-100 hover:border-[#1E4ED8]/30 flex flex-col hover:-translate-y-1.5">
      {/* Image */}
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          <span className={`text-white text-xs font-medium px-3 py-1 rounded-full backdrop-blur-sm ${
            property.status === 'venda' ? 'bg-green-600/90' :
            property.status === 'aluguel' ? 'bg-orange-500/90' :
            'bg-purple-600/90'
          }`}>
        ...
        {property.featured && (
          <div className="absolute top-3 right-3">
            <span className="bg-yellow-500 text-white text-xs font-medium px-3 py-1 rounded-full flex items-center gap-1 shadow-lg">
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="mb-3">
          <p className="text-gradient-blue font-bold text-xl mb-1">
        ...
        <Link
          ...
          className="group/btn mt-auto flex items-center justify-center gap-2 w-full text-center bg-[#0A2A66] hover:bg-[#1E4ED8] text-white text-sm font-medium py-2.5 rounded-xl transition-colors duration-200"
        >
          Ver Detalhes
